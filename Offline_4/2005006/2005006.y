...
start : program 
    {
        ...
        // passing the global variables to the code generator
        $$->setGlobalVariables(globalVariables);
        $$->hasPrintStatement = hasPrintStatement;
        // Generating the assembly code
        $$->generateCode(asmCodeOut);
    }
    ;
...
compound_statement : LCURL 
    {
        ...
        if(params.size()>0){
            ...
            for(SymbolInfo* param : params){
                ...
                stackOffset -= 2;
                tempParam->setStackOffset(stackOffset);
                symbolTable->Insert(tempParam);
            }
            params.clear();
        }
        symbolTable->setCurrentScopeStackOffset(0);
    }
    statements RCURL
    {
        ...
        $$->setStackOffset(symbolTable->getStackOffset());
...
var_declaration : type_specifier declaration_list SEMICOLON
    {
        ...
        bool inGlobalScope = symbolTable->getCurrentScopeID() == 1 ? true : false;
        $$->setGlobalStatus(inGlobalScope);
        ...
        if($1->getType()== "VOID"){
            ...
            for(SymbolInfo* var : $2->getParameterList()){
                ...
                // storing the global variables so that they can be passed at once
                if(inGlobalScope){
                    var->setStackOffset(0);
                    globalVariables.push_back(var);
                    var->setGlobalStatus(true);
                }
                else{
                    var->setGlobalStatus(false);
                    if(var->getArrayStatus()){
                        symbolTable->setCurrentScopeStackOffset(symbolTable->getStackOffset() + (var->getArraySize()*2));
                    }
                    else{
                        symbolTable->setCurrentScopeStackOffset(symbolTable->getStackOffset() + 2);
                    
                    }

                    var->setStackOffset(symbolTable->getStackOffset());
                }
            }
        }
        logOut<<$$->getRule()<<endl;
...
statement : var_declaration
    ...
    | PRINTLN LPAREN ID RPAREN SEMICOLON
    {
        ...
        idNode->setRule("ID : "+$3->getName());
        idNode->name = $3->getName();
        lparenNode->setRule("ID : "+$3->getName());
        Node* rparenNode = new Node(@4.first_line,@4.last_line, true);
        rparenNode->setRule("RPAREN : )");
        Node* semicolonNode = new Node(@5.first_line,@5.last_line, true);
        semicolonNode->setRule("SEMICOLON : ;");
        ...
        $$->setGlobalStatus(idSymbol->getGlobalStatus());
        $$->setStackOffset(idSymbol->getStackOffset());
...
factor : variable
    ...
    | CONST_FLOAT
    {
        ...
        const_floatNode->name = $1->getName();
...
int main(int argc,char *argv[]){
    ...
    asmCodeOut.open("2005006_asmCode.asm");
    /* cout<<"ASM khulse!\n"; */
